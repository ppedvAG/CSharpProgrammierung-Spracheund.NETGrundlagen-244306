...
class Program
{
    static void Main(string[] args)
    {
        ...
        using(StreamWriter sw = new StreamWriter("fahrzeuge.json"))
        {
            
            for (int i = 0; i < 10; i++)
            {
                PKW auto = new PKW("blau", "Benzin", 180, 5);
                
                if(i == 9)
                {
                    sw.WriteLine(JsonSerializer.Serialize(auto));
                    break;
                }
                sw.Write(JsonSerializer.Serialize(auto)+",");
            }
        }
        using (StreamReader sr = new StreamReader("fahrzeuge.json"))
        {
            string json = sr.ReadToEnd();
            List<PKW> autos = JsonSerializer.Deserialize<List<PKW>>(json);
            foreach(PKW auto in autos)
            {
        ...
        /*try
        {
            ...
            Auto auto = new Auto("blau", "Benzin", 50, 5);
        } catch(SpeedUnderZeroException e)
        {
